...
def setup():
    ...
    kw_extractor = yake.KeywordExtractor(top=keywords_in_sig, stopwords=None, lan='nl')
...
def train_func(train_titles, wd, kw_extractor, writer2):
    # Create LSH index
    ...
    with progressbar.ProgressBar(max_value=len(train_titles), widgets=widgets) as bar:
        for title in train_titles:
            ...
            try:
                ...
                row = [content_size, percent_query_datapoints, shingle_size, minhash_num_perm, minhash_threshold,
                       number_of_results, keywords_in_sig,title, preprocessing_delay + t2-t1, preprocessing_delay + t3-t2]
    ...
    with progressbar.ProgressBar(max_value=len(query_titles), widgets=widgets) as bar:
        for query in query_titles:
            ...
            try:
                ...
                row = [content_size, percent_query_datapoints, shing, minhash_num_perm, minhash_threshold,
                       number_of_results, keywords_in_sig , query, shingle_delay,
                       lsh_band_delay, sig_delay, keyword_delay, lsh_band_recall, sig_recall, keyword_recall,
                       shingle_size, sig_size, key_sig_size]
...
def main():
    ...
    runs = [500, 1000, 2000, 5000]
    kwsize = [15, 25, 35]
    topn = [3,5,7]
    
    for run in runs:
        print("Starting runs for size " + str(run) + "...")
        global content_size
        content_size = run
        for kws in kwsize:
            print("Starting runs for keyword size " + str(kws) + "...")
            global keywords_in_sig
            keywords_in_sig = kws
            for n in topn:
                ...
                f1 = open('performance_search_KW.csv', 'a')
                writer1 = csv.writer(f1)
                f2 = open('delay_search_KW.csv', 'a')
                writer2 = csv.writer(f2)

                lsh, train_ds = train_func(train, wd, kwe, writer2)
